...
class NRE_Trainer:
    ...
    def evaluation(self, test = False):
        ...
        return accuracy_score(pred_tags, valid_tags), total_loss/len(self.val_dataloader)
    ...
    def training_process(self, 
        ...
        for epoch in range(0, max_epoch):  # loop over the dataset multiple times
            ...
            if lossmin > avg_valid_loss:
                ...
                if verbose:
                    print(f" time: {time_elapsed // 60:.0f}m {time_elapsed % 60:.0f}s | epoch:{epoch} train loss: {avg_train_loss:.4f} | validation loss: {avg_valid_loss:.4f} acc: {acc:.4f}", end='')   
            else:
                earlystop+=1
                if verbose:
                    print(f" time: {time_elapsed // 60:.0f}m {time_elapsed % 60:.0f}s | epoch:{epoch} train loss: {avg_train_loss:.4f} | validation loss: {avg_valid_loss:.4f} acc: {acc:.4f} -- {earlystop}", end='')   
