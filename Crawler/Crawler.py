...
class Crawler:
    ...
    def search(self, dom, page, link, verbose=False):
        ...
        try:
            ...
            if (
                dom == "www.mirrormedia.mg"
                or dom == "hk.on.cc"
                or dom == "mops.twse.com.tw"
            ):
    ...
    def get_lines(self, dom, page, link):
        ...
        if dom == "udn.com":
            ...
            if "amp_news" in link:
                lines = page.find("div", class_=self.class_name[dom]["amp"]).find_all(
                    "p"
                )
            else:
                lines = page.find("div", class_=self.class_name[dom]["news"]).find_all(
                    "p"
                )
