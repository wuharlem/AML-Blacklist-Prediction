...
def main(args):
    ...
    print(
        f"device: {trainer.device} classification acc: {acc: .4f} validation loss: {total_loss:.4f}"
    )
...
def _parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Blacklist Prediction Model 1 Training"
    )
    parser.add_argument(
        "--save_paths",
        type=str,
        default=f"{current_path}/model_1/params/best-model.pth",
        help="the path of saved parameter",
    )
    parser.add_argument(
        "--dataset",
        type=str,
        default=f"{current_path}/model_1/pkl",
    ...
    parser.add_argument(
        "--weights",
        type=str,
        default=f"{current_path}/model_1/params/pretrained/best-model.pth",
